...
echo " Если вы загрузились в UEFI тогда "1" если BIOS тогда "2" "
echo " Режим BIOS только для mbr-таблицы разделов, UEFI для Gpt-таблицы разделов" 
echo   ""
echo "#################################################"
...
############### disk K ##############
echo 'Добавим раздел диск "K" Windows?'
while 
    read -n1 -p  "
    1 - да
    
    0 - нет: " diskK # sends right after the keypress
    echo ''
    [[ "$diskK" =~ [^10] ]]
do
    :
...
if [[ $diskK == 1 ]]; then
 ...
 lsblk -f
  echo ""
  read -p " Укажите диск "K" раздел(sda/sdb 1.2.3.4 (sda5 например)) : " diskDk
  mkdir /mnt/D 
  mount /dev/$diskDk /mnt/D
  elif [[ $diskK == 0 ]]; then
  echo 'пропущено'
  fi
  fi
...
echo "Добро пожаловать в установку ArchLinux режим BIOS "
lsblk -f
echo ""
echo " Выбирайте "1", если ранее не производилась разметка диска и у вас нет разделов для ArchLinux "
echo ""
...
do
 ...
 if [[ $x_pacstrap == 1 ]]; then
  clear
  pacstrap /mnt base linux linux-headers dhcpcd  base-devel  wget linux-firmware nano 
  genfstab -pU /mnt >> /mnt/etc/fstab
elif [[ $x_pacstrap == 2 ]]; then
  clear
  pacstrap /mnt base dhcpcd linux linux-headers base-devel  wget linux-firmware  nano
  genfstab -pU /mnt >> /mnt/etc/fstab
fi 
 clear
###############################
